...
inline void pop_individual_init(pop_individual *p, bool reset_curr_physical_num) {
    ...
    (*p).curr_cost = 0.0;
    if (reset_curr_physical_num) {
        (*p).curr_physical_num = 0;
        for (int j = 0; j < MAX_PHYSICAL_NUM; ++j) {
            Physical_Node_Init(&(*p).Physical_play_list[j]);
        }
    } else {
        for (int j = (*p).curr_physical_num; j < MAX_PHYSICAL_NUM; ++j) {
            for (int k = 0; k < PHYSICAL_MAX_CONTAIN; k++) {
                if (p->Physical_play_list[j].contain_Template_Index[k] != -1) {
                    printf("ERR");
                }
            }
        }
        for (int j = 0; j < (*p).curr_physical_num; ++j) {
            Physical_Node_Init(&(*p).Physical_play_list[j]);
        }
        (*p).curr_physical_num = 0;
    }
        ...
        if (belong_flag == 0) {
            for (int ser_num = 0; ser_num < All_Count; ++ser_num) {
                if (ans.Physical_play_list[ser_num].remain_cpu - cpu_consume[play_flavor_type - 1] >= 0 &&
                    ...
                    int free_pos = -1;
                    free_pos = find_free_pos(ans.Physical_play_list[ser_num].contain_Template_Index,
                                             PHYSICAL_MAX_CONTAIN);
                    if (free_pos == -1) {
...
//                rand select
            ...
            ans.Physical_play_list[All_Count].remain_cpu =
                    Physical_Info_Arr[rand_select].cpu_value - cpu_consume[play_flavor_type - 1];
            ans.Physical_play_list[All_Count].remain_mem =
                    Physical_Info_Arr[rand_select].mem_value - mem_consume[play_flavor_type - 1];
...
void POP_Init(int POP_size, const int *Template, int Template_size) {
    ...
    if (pop_list != NULL) {
        for (int i = 0; i < POP_size; ++i) {
            pop_individual_init(pop_list + i, true);
        }
        for (int i = 0; i < POP_size; ++i) {
...
//    init GA_X
    ...
    pop_individual_init(&GA_X, false);
...
//            遍历该箱子中是否存在unqiue_list元素,如果存在则再下面进行将箱子未重复的元素倒入到unplay_list中
            ...
            temp_list.clear();
            for (g = 0, k = 0;
                 k < PHYSICAL_MAX_CONTAIN && g < pop_list[Y_index].Physical_play_list[i].curr_contain_num; ++k) {
...
//            temp_unplay_list.clear();
            if (play_flag == 1) {
                unplay_list.insert(temp_list.begin(), temp_list.end());
//                set_union(unplay_list.begin(), unplay_list.end(), temp_list.begin(), temp_list.end(),
//                          inserter(temp_unplay_list, temp_unplay_list.begin()));
//                unplay_list.clear();
...
//                删除箱子
                int m, n;
                for (m = 0, n = 0;
                     m < PHYSICAL_MAX_CONTAIN && n < pop_list[X_index].Physical_play_list[i].curr_contain_num; ++m) {
        ...
        if (belong_flag == 0) {
            for (int ser_num = 0; ser_num < All_Count; ++ser_num) {
                if ((*X).Physical_play_list[ser_num].remain_cpu - cpu_consume[unplay_flavor_type - 1] >= 0 &&
                    ...
                    int free_pos = -1;
                    free_pos = find_free_pos((*X).Physical_play_list[ser_num].contain_Template_Index,
                                             PHYSICAL_MAX_CONTAIN);
                    if (free_pos == -1) {
                        fprintf(stderr, "can not find free pos in Physical_play_list[%d]->contain_template_index\n",
                                ser_num);
...
//                rand select
            ...
            (*X).Physical_play_list[All_Count].remain_cpu =
                    Physical_Info_Arr[rand_select].cpu_value - cpu_consume[unplay_flavor_type - 1];
            (*X).Physical_play_list[All_Count].remain_mem =
                    Physical_Info_Arr[rand_select].mem_value - mem_consume[unplay_flavor_type - 1];
...
/************************遗传算法的主函数****************************/
Result GAA_main(int *pre_flavor_arr, int pre_flavor_arr_size, const pre_flavor_info flavor_info,
                const limit_info *physical_info, int physical_info_size, const int size, const int max_iter,
